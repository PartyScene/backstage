...
async def livestream_app(redis_connection):
...
async def livestream_client(livestream_app, bearer):
    """Create an async HTTP client for testing."""
    try:
        async with livestream_app.test_client() as test_client:
            test_client.headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {bearer}",
            }
            async with livestream_app.app_context():
                yield test_client
    except Exception as e:
