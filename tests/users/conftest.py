...
async def users_app(redis_connection):
...
async def users_client(users_app, bearer):
    """Create an async HTTP client for testing."""
    try:
        async with users_app.test_client() as test_client:
            test_client.headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {bearer}",
            }
            async with users_app.app_context():
                yield test_client
    except Exception as e:
        logger.error(f"Error in users_client fixture: {str(e)}")
        raise
