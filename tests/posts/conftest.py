...
def mock_media_client():
    """Mock the MediaClient for posts tests"""
...
async def posts_client(posts_app, bearer):
    """Create an async HTTP client for testing."""
    try:
        async with posts_app.test_client() as test_client:
            test_client.headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {bearer}",
            }
            async with posts_app.app_context():
                yield test_client
    except Exception as e:
        logger.error(f"Error in posts_client fixture: {str(e)}")
        raise
