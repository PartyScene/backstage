...
class TestPostOperations:
    async def test_create_post(self, async_client, test_config):
        """Test creating a new post."""
        post_data = {
            "title": fake.sentence(),
            "content": fake.text(),
            "tags": [fake.word() for _ in range(3)],
            "author_id": test_config['test_user']['id'],
            "visibility": "public"
        }
    ...
    async def test_get_post(self, async_client, test_config):
    ...
    async def test_delete_post(self, async_client):
    ...
    async def test_create_invalid_post(self, async_client, invalid_data):
        """Test post creation with invalid data."""
        response = await async_client.post("/posts", json=invalid_data)
