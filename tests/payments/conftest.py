...
async def payments_app(redis_connection):
    from payments.run import app
    ...
    try:
        ...
        logger.error(f"Error in payments_app fixture: {str(e)}")
        raise
    finally:
        # Clean up resources
...
async def payments_client(payments_app, bearer):
    """Create an async HTTP client for testing."""
    try:
        async with payments_app.test_client() as test_client:
            test_client.headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {bearer}",
            }
            async with payments_app.app_context():
                yield test_client
    except Exception as e:
        logger.error(f"Error in payments_client fixture: {str(e)}")
        raise
