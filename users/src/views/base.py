...
class BaseView(QuartClassful):
    ...
    async def get_me(self) -> Tuple[Dict[str, Any], int]:
        ...
        try:
            ...
            if not user:
                ...
                return jsonify(message="User not found", status=status_code.phrase), status_code
            status_code = HTTPStatus.OK
            return jsonify(data=user, message="User details fetched successfully.", status=status_code.phrase), status_code
        except Exception as e:
            app.logger.error(f"Error fetching current user ({user_id}): {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to fetch user details: {str(e)}", status=status_code.phrase), status_code
    ...
    async def delete_me(self) -> Tuple[Dict[str, Any], int]:
        ...
        try:
            ...
            return jsonify(message="User deleted successfully", status=status_code.phrase), status_code
        except Exception as e:
            app.logger.error(f"Error deleting current user ({user_id}): {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to delete user: {str(e)}", status=status_code.phrase), status_code
    ...
    async def update_me(self) -> Tuple[Dict[str, Any], int]:
        ...
        try:
            ...
            if not data:
                 ...
                 return jsonify(message="Request body required.", status=status_code.phrase), status_code
            ...
            if not result: # Handle case where update fails or user doesn't exist
                 ...
                 return jsonify(message="User not found or update failed.", status=status_code.phrase), status_code # Or BAD_REQUEST?

            status_code = HTTPStatus.OK
            return jsonify(data=result, message="User updated successfully.", status=status_code.phrase), status_code
        except Exception as e:
            app.logger.error(f"Error updating current user ({user_id}): {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to update user: {str(e)}", status=status_code.phrase), status_code
    ...
    async def get_user(self, user_id: str):
        ...
        try:
            ...
            status_code = HTTPStatus.OK
            return jsonify(data=user, message="User profile fetched successfully.", status=status_code.phrase), status_code
        except Exception as e:
            app.logger.error(f"Error fetching user profile ({user_id}): {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to fetch user profile: {str(e)}", status=status_code.phrase), status_code
    ...
    async def search_user(self):
        ...
        if not username:
            status_code = HTTPStatus.BAD_REQUEST
            return jsonify(message="Username query parameter is required.", status=status_code.phrase), status_code
        ...
        return jsonify(message="Search endpoint not yet implemented.", status=status_code.phrase), status_code
    ...
    async def get_connections_at_degree(self): # Removed extra bracket
        ...
        user_id = get_jwt_identity() # Get user_id early for logging
        try:
            max_degree = request.args.get("max_degree", type=int, default=1) # Default to 1

            if not 1 <= max_degree <= 6:
                status_code = HTTPStatus.BAD_REQUEST
                return jsonify(message="max_degree must be between 1 and 6", status=status_code.phrase), status_code

            result = await self.conn.find_connections_at_degree(user_id, max_degree)
            status_code = HTTPStatus.OK
            return jsonify(data=result, message=f"Connections up to degree {max_degree} fetched.", status=status_code.phrase), status_code
        except ValueError: # Catch potential type error for max_degree
             status_code = HTTPStatus.BAD_REQUEST
             return jsonify(message="Invalid value for max_degree parameter.", status=status_code.phrase), status_code
        except Exception as e:
            app.logger.error(f"Error fetching connections for user {user_id}: {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to fetch connections: {str(e)}", status=status_code.phrase), status_code
    ...
    async def create_connection(self):
        ...
        try:
            ...
            if not target_id:
                 status_code = HTTPStatus.BAD_REQUEST
                 return jsonify(message="target_id is required in request body.", status=status_code.phrase), status_code
            ...
            if data["origin_id"] == target_id:
                 status_code = HTTPStatus.BAD_REQUEST
                 return jsonify(message="Cannot send friend request to yourself.", status=status_code.phrase), status_code

            if result := await self.conn.create_friend_relationship(data):
                app.logger.info(f"Friend request created: {json.dumps(result, option=json.OPT_INDENT_2, default=str)}")
                ...
                if isinstance(result, dict) and "in" in result and "out" in result:
                    try:
                        ...
                        app.logger.error(f"Failed to send friend request notification: {notify_err}")

                status_code = HTTPStatus.CREATED
                message = "Friend request sent successfully." if notification_sent else "Friend request created, but notification failed."
                return jsonify(data=result, message=message, status=status_code.phrase), status_code

            # Handle case where connector returns non-truthy without exception
            app.logger.warning(f"Failed to create friend request between {data['origin_id']} and {target_id} (connector returned non-truthy)")
            status_code = HTTPStatus.BAD_REQUEST # Or CONFLICT?
            return jsonify(message="Failed to create friend request (e.g., already exists or invalid IDs).", status=status_code.phrase), status_code # Or CONFLICT?

        except Exception as e:
            app.logger.error(f"Error creating friend request: {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to send friend request: {str(e)}", status=status_code.phrase), status_code
    ...
    async def update_connection(self, connection_id: str):
        ...
        try:
            ...
            allowed_statuses = ["accepted", "blocked", "pending"] # Include pending if needed
            if new_status not in allowed_statuses:
                 status_code = HTTPStatus.BAD_REQUEST
                 return jsonify(message=f"Invalid status. Must be one of: {', '.join(allowed_statuses)}", status=status_code.phrase), status_code
            ...
            if result: # Assuming update returns the updated connection or True
                ...
                status_code = HTTPStatus.OK
                return jsonify(data=result, message="Connection status updated successfully.", status=status_code.phrase), status_code
            else:
                 app.logger.warning(f"Failed to update connection {connection_id} (connector returned non-truthy)")
                 status_code = HTTPStatus.NOT_FOUND
                 return jsonify(message="Connection not found or update failed.", status=status_code.phrase), status_code

        except Exception as e:
            app.logger.error(f"Error updating connection {connection_id}: {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to update connection: {str(e)}", status=status_code.phrase), status_code
    ...
    async def delete_connection(self, connection_id: str):
        ...
        try:
            ...
            if result: # Assuming delete returns True or affected count > 0
                status_code = HTTPStatus.NO_CONTENT
                return jsonify(message="Connection deleted successfully.", status=status_code.phrase), status_code
            else:
                 app.logger.warning(f"Connection {connection_id} not found or deletion failed.")
                 status_code = HTTPStatus.NOT_FOUND
                 return jsonify(message="Connection not found or could not be deleted.", status=status_code.phrase), status_code

        except Exception as e:
            app.logger.error(f"Error deleting connection {connection_id}: {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to delete connection: {str(e)}", status=status_code.phrase), status_code
    ...
    async def upload_media(self):
        ...
        try:
            ...
            if not file or not file.filename:
                status_code = HTTPStatus.BAD_REQUEST
                return jsonify(message="No file provided or file has no name.", status=status_code.phrase), status_code
            ...
            app.logger.warning(f"Publishing user media upload task to RMQ: {rmq_data['filename']}")
            ...
            return jsonify(message="Media upload process initiated successfully.", status=status_code.phrase), status_code # 202 Accepted is suitable here

        except Exception as e:
            app.logger.error(f"Error initiating media upload for user {user_id}: {str(e)}", exc_info=True)
            status_code = HTTPStatus.INTERNAL_SERVER_ERROR
            return jsonify(message=f"Failed to initiate media upload: {str(e)}", status=status_code.phrase), status_code
