...
class LSClient:
    ...
    async def get_stream(self, event: str):
        ...
        """
        self.logger.info(
            "FETCHING STREAM DETAILS (CHANNEL, PLAYBACK_URL, INGEST_URL) FOR LIVESTREAM FOR EVENT %s"
            % event
        )
    ...
    async def _create_input(self) -> Input:
        ...
        operation = await self.client.create_input(
            parent=self.get_parent(), input=input_config, input_id=input_id
        )
        response = (
            await operation
        ).result()  # Wait for the operation to complete (900 seconds)
        return response
    ...
    async def _create_channel(self, input_name: str) -> Channel:
        ...
        channel = Channel(
            name=channel_id,
            input_attachments=[
                InputAttachment(key="%s-input" % (channel_id), input=input_name)
            ],
            elementary_streams=[
                ElementaryStream(
                ...
                ElementaryStream(
                    ...
                    audio_stream=AudioStream(bitrate_bps=1000000),
                ),
            ],
            mux_streams=[
                MuxStream(
            ...
            manifests=[
                Manifest(
                    ...
                    mux_streams=[
                        "%s_mux_video" % channel_id,
                        "%s_mux_audio" % channel_id,
                    ],
                )
            ],
        ...
        # Make the request
        operation = await self.client.create_channel(
            parent=self.get_parent(), channel=channel, channel_id=channel_id
        )
